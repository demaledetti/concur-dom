...
import           Concur.DOM                   (HTML, el, text, interval,
                                               runWidgetInBody)
...
main :: IO ()
main = void $ runWidgetInBody mario
...
drawMario m@Mario{..} = el E.div [ A.height h, A.width w ]
  ...
  where
    ...
    d = case dir of
      L -> "left"
      R -> "right"
    src = "https://ajnsit.github.io/concur/examples/imgs/" ++ verb ++ "/" ++ d ++ ".gif"
    gy = 62 - (fromIntegral w / 2)
    marioStyle = "display:block; transform: " ++ matrix x (abs (y + gy))
